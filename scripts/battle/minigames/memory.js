...
export default {
  ...
  play(area, ctx) {
    return new Promise(resolve => {
      const len = Math.min(10, 2 + Math.floor(ctx.difficulty * 0.8));  // 6 on Normal
      ...
      const light = i => { pads[i].classList.add('lit'); S.rune(i); setTimeout(() => pads[i].classList.remove('lit'), showMs * 0.6); };
