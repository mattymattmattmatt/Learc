...
export default {
  ...
  play(area, ctx) {
    return new Promise(resolve => {
      ...
      const rate = 0.55 + ctx.difficulty * 0.07;         // meter fill per second
      const bandW = clamp(0.26 - ctx.difficulty * 0.016, 0.1, 0.26);
      const bandLo = 0.66, bandHi = bandLo + bandW;       // golden band near the top

      area.innerHTML = `
        ${stageHTML(ctx, 'ch')}
        <div class="ps-hp">Foe HP <b id="hp">${'❤'.repeat(hp)}</b></div>
        <div class="ch-meter" id="meter">
          <div class="ch-band" style="bottom:${bandLo*100}%;height:${bandW*100}%"></div>
